...
async function isIdentifierBanned(email, mobile) {
    if (email) {
        ...
        if (user?.isBanned)
            return { banned: true, contact: email };
        const bannedRecord = await prisma_1.prisma.bannedUser.findUnique({ where: { email } });
        if (bannedRecord)
    ...
    if (mobile) {
        ...
        if (user?.isBanned)
            return { banned: true, contact: mobile };
        const bannedRecord = await prisma_1.prisma.bannedUser.findFirst({ where: { mobile } });
        if (bannedRecord)
            return { banned: true, contact: bannedRecord.email, reason: bannedRecord.reason };
    }
    return { banned: false };
...
router.post("/register", async (req, res) => {
    try {
        ...
        await prisma_1.prisma.pendingRegistration.deleteMany({ where: { email } });
        const hashedPassword = await bcryptjs_1.default.hash(password, 12);
        const otpCode = generateOtp();
        await prisma_1.prisma.pendingRegistration.create({
            data: {
                name,
                email,
                mobile,
        ...
        await (0, mailer_1.sendOtpEmail)(email, otpCode);
...
router.post("/verify-otp", async (req, res) => {
    try {
        ...
        const pending = await prisma_1.prisma.pendingRegistration.findUnique({ where: { email } });
        if (!pending)
            return res.status(400).json({ success: false, message: "No pending registration" });
        if (pending.otpCode !== otp)
            return res.status(400).json({ success: false, message: "Invalid OTP" });
        if (pending.expiresAt < new Date()) {
            await prisma_1.prisma.pendingRegistration.delete({ where: { email } });
        ...
        await prisma_1.prisma.pendingRegistration.delete({ where: { email } });
        await (0, mailer_1.sendWelcomeEmail)(user.email, user.name || undefined);
...
router.post("/oauth-sync", async (req, res) => {
    try {
        ...
        if (isNew)
            await (0, mailer_1.sendWelcomeEmail)(email, name || undefined);
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || "fallback_secret", { expiresIn: "7d" });
        return res.json({
            success: true,
            user: {
...
router.get("/check-banned", async (req, res) => {
