...
function handleReviewImageUpload(req, res, next) {
...
function getProductReviewDelegate(db) {
    return db.productReview || db.review;
}
function getProductReviewImageDelegate(db) {
...
function requiredNumberFromBody(value, fieldName) {
...
function intFromBody(value, fallback = 0) {
...
function normalizeProductType(value) {
    const valid = Object.values(client_1.ProductType);
    if (typeof value === "string" && valid.includes(value)) {
        return value;
    }
    return client_1.ProductType.single;
}
function normalizeStockStatus(value) {
    const valid = Object.values(client_1.StockStatus);
    if (typeof value === "string" && valid.includes(value)) {
        return value;
    }
...
const purchasableStockStatuses = [
    client_1.StockStatus.IN_STOCK,
    client_1.StockStatus.LIMITED_STOCK,
    client_1.StockStatus.LOW_STOCK,
    client_1.StockStatus.PRE_ORDER,
];
function canAddToCartByStatus(stockStatus) {
...
function getExtraImageFiles(files) {
    return Object.entries(files)
        .filter(([fieldName]) => {
        return (fieldName === "extraImages" ||
...
async function createUniqueSlug(baseText, model, categoryId, ignoreId) {
    ...
    while (true) {
        let existing = null;
        if (model === "product") {
            existing = await prisma_1.prisma.product.findUnique({ where: { slug } });
        }
        if (model === "category") {
            existing = await prisma_1.prisma.productCategory.findUnique({ where: { slug } });
        }
        if (model === "brand") {
            existing = await prisma_1.prisma.brand.findUnique({ where: { slug } });
        }
        if (model === "subCategory") {
            if (!categoryId)
                throw new Error("categoryId required for sub-category slug");
            existing = await prisma_1.prisma.productSubCategory.findUnique({
                where: {
                    categoryId_slug: {
                        categoryId,
                        slug,
...
async function resolveCategoryId(body) {
    ...
    const existing = await prisma_1.prisma.productCategory.findUnique({
        where: { slug: baseSlug },
    });
    if (existing)
        return existing.id;
...
async function resolveSubCategoryId(body, categoryId) {
    ...
    if (subCategoryId) {
        const subCategory = await prisma_1.prisma.productSubCategory.findUnique({
    ...
    const existing = await prisma_1.prisma.productSubCategory.findUnique({
        where: {
            categoryId_slug: {
                categoryId,
                slug: baseSlug,
    ...
    const subCategory = await prisma_1.prisma.productSubCategory.create({
        data: {
...
function requiredReviewRating(value) {
...
async function getPublishedProductReviews(productId) {
    const reviewDelegate = getProductReviewDelegate(prisma_1.prisma);
...
async function findPublishedProductBySlugOrId(slugOrId) {
...
function serializeSizeChart(sizeChart) {
    if (!sizeChart)
        return null;
    return {
        id: sizeChart.id,
...
function serializeProduct(product, options = {}) {
    ...
    const serialized = {
        ...
        freeDeliveryMinAmount: product.freeDeliveryMinAmount === null ? null : Number(product.freeDeliveryMinAmount),
...
function buildProductData(body, args) {
...
async function handleSizeChart(productId, body, files, existingSizeChart) {
    const removeSizeChart = booleanFromBody(body.removeSizeChart, false);
    if (removeSizeChart) {
        if (existingSizeChart?.cloudinaryPublicId) {
            await (0, cloudinary_1.deleteFromCloudinary)(existingSizeChart.cloudinaryPublicId, "image");
        }
        await prisma_1.prisma.sizeChart.deleteMany({ where: { productId } });
        return;
    }
    const chartData = parseJson(body.sizeChartData);
    const hasSizeChartData = optionalString(body.sizeChartName) ||
    ...
    if (booleanFromBody(body.removeSizeChartImage, false) && cloudinaryPublicId) {
        await (0, cloudinary_1.deleteFromCloudinary)(cloudinaryPublicId, "image");
        imageUrl = null;
        cloudinaryPublicId = null;
    }
    const sizeChartImage = files.sizeChartImage?.[0];
    if (sizeChartImage) {
        const uploaded = await (0, cloudinary_1.uploadImageToCloudinary)(sizeChartImage.buffer, "digital-xpress/products/size-charts");
    ...
    await prisma_1.prisma.sizeChart.upsert({
        where: { productId },
        update: {
...
router.get("/meta", async (_req, res) => {
    try {
        ...
        return res.json({
            ...
            categories,
            brands,
            productTypes: Object.values(client_1.ProductType),
            stockStatuses: Object.values(client_1.StockStatus),
        });
    }
    catch (error) {
...
router.get("/admin", auth_1.authenticate, auth_1.requireAdminOrModerator, async (req, res) => {
    try {
        ...
        if (stockStatus && Object.values(client_1.StockStatus).includes(stockStatus)) {
            where.stockStatus = stockStatus;
        }
        if (productType && Object.values(client_1.ProductType).includes(productType)) {
...
router.get("/admin/:id", auth_1.authenticate, auth_1.requireAdminOrModerator, async (req, res) => {
    try {
...
router.get("/", async (req, res) => {
    try {
        ...
        const minPrice = req.query.minPrice !== undefined ? numberFromBody(req.query.minPrice) : undefined;
        const maxPrice = req.query.maxPrice !== undefined ? numberFromBody(req.query.maxPrice) : undefined;
        const where = {
            isPublished: true,
        };
        if (search) {
...
router.post("/", auth_1.authenticate, auth_1.requireAdminOrModerator, productUpload, async (req, res) => {
    try {
        ...
        const [mainUploaded, hoverUploaded, videoUploaded] = await Promise.all([
            (0, cloudinary_1.uploadImageToCloudinary)(mainImage.buffer, "digital-xpress/products/main"),
            files.hoverImage?.[0]
                ? (0, cloudinary_1.uploadImageToCloudinary)(files.hoverImage[0].buffer, "digital-xpress/products/hover")
                : Promise.resolve(null),
            files.video?.[0]
                ? (0, cloudinary_1.uploadVideoToCloudinary)(files.video[0].buffer, "digital-xpress/products/videos")
                : Promise.resolve(null),
        ]);
        ...
        if (extraImages.length > 0) {
            const uploadedImages = await Promise.all(extraImages.map((file) => (0, cloudinary_1.uploadImageToCloudinary)(file.buffer, "digital-xpress/products/extra")));
        ...
        const createdProduct = await prisma_1.prisma.product.findUnique({
            where: { id: product.id },
            include: productInclude,
        });
        if (!createdProduct) {
...
router.patch("/:id", auth_1.authenticate, auth_1.requireAdminOrModerator, productUpload, async (req, res) => {
    try {
        const productId = getStringParam(req.params.id);
        if (!productId) {
            return res.status(400).json({ success: false, message: "Product ID is required" });
        ...
        const brandId = await resolveBrandId(req.body);
        const customSlug = optionalString(req.body.slug);
        const slug = await createUniqueSlug(customSlug || name, "product", undefined, existingProduct.id);
        ...
        if (mainImage) {
            const uploaded = await (0, cloudinary_1.uploadImageToCloudinary)(mainImage.buffer, "digital-xpress/products/main");
            if (mainImagePublicId) {
                await (0, cloudinary_1.deleteFromCloudinary)(mainImagePublicId, "image");
            }
        ...
        if (booleanFromBody(req.body.removeHoverImage, false) && hoverImagePublicId) {
            await (0, cloudinary_1.deleteFromCloudinary)(hoverImagePublicId, "image");
            hoverImageUrl = null;
            hoverImagePublicId = null;
        }
        if (hoverImage) {
            const uploaded = await (0, cloudinary_1.uploadImageToCloudinary)(hoverImage.buffer, "digital-xpress/products/hover");
        ...
        if (video) {
            const uploaded = await (0, cloudinary_1.uploadVideoToCloudinary)(video.buffer, "digital-xpress/products/videos");
            if (videoPublicId) {
                await (0, cloudinary_1.deleteFromCloudinary)(videoPublicId, "video");
            }
        ...
        if (removeExtraImageIds.length > 0) {
            const imagesToRemove = await prisma_1.prisma.productImage.findMany({
                where: {
                    productId: existingProduct.id,
                    id: { in: removeExtraImageIds },
                },
            });
            await Promise.all(imagesToRemove.map((image) => (0, cloudinary_1.deleteFromCloudinary)(image.cloudinaryPublicId, "image")));
        ...
        const extraImages = getExtraImageFiles(files);
        if (extraImages.length > 0) {
            const existingExtraCount = await prisma_1.prisma.productImage.count({
...
router.patch("/:id/stock-status", auth_1.authenticate, auth_1.requireAdminOrModerator, async (req, res) => {
    try {
...
router.patch("/:id/flags", auth_1.authenticate, auth_1.requireAdminOrModerator, async (req, res) => {
    try {
        const productId = getStringParam(req.params.id);
        if (!productId) {
            return res.status(400).json({ success: false, message: "Product ID is required" });
...
function getReviewImageFiles(req) {
    const files = req.files;
    return Array.isArray(files) ? files : [];
}
async function uploadReviewImages(files) {
    return Promise.all(files.map((file) => (0, cloudinary_1.uploadImageToCloudinary)(file.buffer, "digital-xpress/products/reviews")));
}
async function cleanupUploadedReviewImages(images) {
    await Promise.allSettled(images.map((image) => (0, cloudinary_1.deleteFromCloudinary)(image.public_id, "image")));
}
...
router.post("/:slugOrId/reviews", auth_1.authenticate, handleReviewImageUpload, async (req, res) => {
    ...
    try {
        ...
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            return {
                ...
                averageRating,
                totalReviews,
                oldImagesToDelete: uploadedReviewImages.length > 0 ? existingReview?.images || [] : [],
            };
        });
        if (result.oldImagesToDelete.length > 0) {
            await Promise.allSettled(result.oldImagesToDelete.map((image) => (0, cloudinary_1.deleteFromCloudinary)(image.cloudinaryPublicId, "image")));
        }
        return res.status(201).json({
            success: true,
