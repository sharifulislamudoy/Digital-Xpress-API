...
function calculateDeliveryCharge(
  ...
  const charges = products.map((product: ProductForCheckout) => {
    const freeMin = decimalToNumber(product.freeDeliveryMinAmount);
    if (product.freeDelivery || (freeMin > 0 && totalAmount >= freeMin)) return 0;
...
function normalizeCheckoutItems(itemsInput: unknown): NormalizedCheckoutItem[] {
  ...
  return itemsInput
    .map((rawItem: CheckoutItemInput): NormalizedCheckoutItem => {
      const productId =
    ...
    .filter((item: NormalizedCheckoutItem) => item.productId.length > 0);
}
...
router.post("/checkout", authenticate, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const productIds: string[] = Array.from(
      new Set<string>(normalizedItems.map((item: NormalizedCheckoutItem) => item.productId))
    );
    ...
    const productMap = new Map<string, ProductForCheckout>(
      products.map((product: ProductForCheckout) => [product.id, product])
    );

    const orderItems: Prisma.OrderItemUncheckedCreateWithoutOrderInput[] = normalizedItems.map(
      (item: NormalizedCheckoutItem): Prisma.OrderItemUncheckedCreateWithoutOrderInput => {
        const product = productMap.get(item.productId);

        if (!product) throw new Error("Product not found");
        if (!canBuyProduct(product)) throw new Error(`${product.name} is not available for checkout`);
    ...
    const totalAmount = Number(
      orderItems
        .reduce((sum: number, item: Prisma.OrderItemUncheckedCreateWithoutOrderInput) => {
    ...
    const itemDescription = orderItems
      .map((item: Prisma.OrderItemUncheckedCreateWithoutOrderInput) => `${item.productName} x ${item.quantity}`)
      .join(", ")
      .slice(0, 400);
...
router.patch("/admin/:invoiceNo", authenticate, requireAdminOrModerator, async (req, res) => {
  try {
    ...
    if (!status) return res.status(400).json({ success: false, message: "Invalid order status" });
...
router.delete("/admin/:invoiceNo", authenticate, requireAdminOrModerator, async (req, res) => {
  try {
    ...
    await prisma.order.delete({ where: { invoiceNo } });
